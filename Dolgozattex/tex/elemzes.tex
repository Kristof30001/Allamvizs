...
\textbf{Egy nap részletes esetelemzése:}
...
A különböző algoritmusok bemeneti profilját a \ref{fig:winter_day}. ábra, míg a költségeloszlást a \ref{fig:winter_box}. ábra szemlélteti.
...
\textbf{A 2x2-es kísérleti mátrix: robusztusság validálása:}
...
Ahhoz, hogy az algoritmusok \textit{generalizálhatóságát} is teszteljük, négy kombinációt értékeltünk, amelyeket a \ref{tab:matrix_2x2}. táblázat foglal össze:
...
\textbf{A 2x2-es mátrix tudományos értéke:}
...
\begin{itemize}
	...
	\item 5 futás / profil / algoritmus,
...
\textbf{Gazdasági megtérülés:}
A beruházási és megtérülési becslést a \ref{tab:payback_estimate}. táblázat szemlélteti.

\begin{table}[H]
\centering
\caption{Beruházási és megtérülési kalkuláció becslés}
\label{tab:payback_estimate}
\begin{tabular}{lc}
\toprule
...
\textbf{Jövőbeli kutatási irányok és fejlesztési lehetőségek:}
...
\textbf{Implementációs ajánlások és gazdasági szempontok:}
...
\textbf{Kódbázis-alapú benchmark-értékelés:}
A projekt legfontosabb eredményei a négy szcenáriót vizsgáló, valós és szintetikus adatokat kombináló (2x2) benchmarkból származnak. Ez a futtatás 16 kiválasztott napot, 4 adatváltozatot, 5 algoritmust és 5 ismétlést kombinál, vagyis \textbf{összesen 1600 algoritmusfuttatást} értékel.
...
\textbf{A Hibrid algoritmus alulteljesítésének okai:}
A mérési eredmények rámutatnak egy látszólagos anomáliára: a Hibrid algoritmus mind a négy forgatókönyvben alulmaradt az önálló Genetikus Algoritmushoz képest. Ennek oka a hibridizációs stratégia belső arányaiban és az optimalizálási tájkép zajosságában keresendő. A jelenlegi implementációban a futási keret első 60\%-át a PSO, míg a maradék 40\%-ot a GA kapja. Ahogy a benchmark eredmények mutatják, a PSO a valós, tüskés adatok esetén fokozottan hajlamos a korai stagnálásra (premature convergence). Mivel a keresés több mint felét ez az algoritmus dominálja, a raj gyorsan beragad egy lokális optimumba. Mire a vezérlés átkerül a GA fázisba, a populáció már elveszítette a diverzitását, és a maradék 40\% iteráció nem elegendő a genetikai sokszínűség helyreállítására.

\textbf{A termelési oldal zajának hatása (A Gap \% ugrása):}
A 2. forgatókönyv (Termelési zaj) eredményei rávilágítanak a valós napelemes adatok bizonytalanságainak kritikus hatására. Míg az 1. szcenárióban a GA(P=50) és a GWO relatív hibája (gap) csupán 4.7\%, illetve 10\% volt, addig a valós inverter adatok bevezetésekor ez 32\%-ra és 60\%-ra ugrott. Ennek fizikai oka a valós adatokban megjelenő meteorológiai anomáliák – például a hirtelen felhőátvonulások okozta mikrozuhanások – jelenléte. Ezek az események meredek letöréseket és kiszámíthatatlan lokális minimumokat generálnak a célfüggvényben. A kisebb populációjú GA(P=50) és a kevesebb paraméterrel dolgozó GWO diverzitása ezen a ponton már nem elegendő a folyamatosan széttöredező gradiens követésére.

\textbf{A keresési tér matematikai anatómiája: felfedezés vs. kiaknázás:}

A 640 futásos benchmark egyik legszembetűnőbb eredménye, hogy a PSO a szintetikus, laboratóriumi környezetben még versenyképes volt, azonban a valós fogyasztási adatok és a valós inverter-mérések együttes bevezetésekor a teljesítménye drasztikusan romlott. Ennek oka a megváltozott keresési tér topológiájában és a PSO belső frissítési mechanizmusában keresendő.

\textbf{A zajos adatok hatása a keresési térre (Rugged Landscape):}
A generált, szintetikus profilok folytonos, sima görbéket eredményeznek. Ezek a profilok a célfüggvény terét egy viszonylag jól strukturált, közel konvex jellegű térbe rendezik, ahol a globális optimum felé mutató irányok egyértelműbbek. Ezzel szemben a valós UCI-alapú fogyasztási adatok a háztartási gépek véletlenszerű bekapcsolásából származó hirtelen tüskéket, kiugrásokat és aszimmetriát visznek a bemenetbe. Ez a hatás a keresési teret rücskös, több lokális optimumot tartalmazó, nehezen bejárható tájjá alakítja.

\textbf{A PSO korai konvergenciája:}
A PSO algoritmusban a részecskék sebességét a következő egyenlet határozza meg:
\begin{equation}
v_i^{t+1} = w \cdot v_i^t + c_1 \cdot r_1 \cdot (pbest_i - x_i^t) + c_2 \cdot r_2 \cdot (gbest - x_i^t)
\end{equation}
ahol $pbest_i$ a részecske saját eddigi legjobb pozíciója, a $gbest$ pedig a teljes raj által talált globális legjobb pozíció.

Amennyiben a raj egy zajos idősor miatt egy hamis lokális optimumhoz vonzódik, a $gbest$ komponens gyorsan magához rántja a többi részecskét. Ezzel a diverzitás csökken, a sebességek lelassulnak, és a keresés korán stagnálhat. Ez a viselkedés különösen akkor jelenik meg, amikor a bemenet egyszerre tartalmaz termelési és fogyasztási zajt.

\textbf{A genetikus algoritmus robusztussága:}
Ezzel szemben a genetikus algoritmus a mutációs operátor révén jobban fenntartja a populáció sokszínűségét. A véletlenszerű, gradiensfüggetlen ugrások segítenek abban, hogy az algoritmus ne ragadjon be tartósan egy lokális csapdába. A nagyobb populációméret és a mutáció együttese szélesebb térbejárást tesz lehetővé, ezért a GA különösen a valós, zajos környezetben bizonyult stabilabbnak.

Ebből a benchmarkból az a mérnöki következtetés vonható le, hogy a HEMS-feladatoknál nem csak a gyors konvergencia, hanem a diverzitás fenntartása is kulcsfontosságú. A PSO gyorsasági előnye önmagában nem ellensúlyozza a zajos bemenetekre való érzékenységét, míg a GA kevésbé látványos, de tartósabban megbízható teljesítményt ad.

\textbf{A bemeneti adatminőség hatása az optimalizációs teljesítményre:}
...
\textbf{Akkumulátor-degradáció és a túlzott kiaknázás kezelése:}

A HEMS optimalizáció korai fázisában a kísérletek egy klasszikus metaheurisztikus jelenségre mutattak rá, amelyet a szakirodalom \textit{túlzott kiaknázásnak (over-exploitation)} nevez. Amikor az algoritmusok számára az akkumulátor használata gyakorlatilag költségmentes, a solver kizárólag a hálózati árszámla minimalizálására törekszik. Ennek következménye, hogy a gyorsan konvergáló módszerek, különösen a PSO és a hibrid eljárás, hajlamosak a mikrociklusok generálására: a tárolót rövid időközönként töltik és kisütik, miközben a valós pénzügyi nyereség alig néhány bani.

\textbf{A mikrociklusok problémája:}

A valós ENTSO-E áradatok gyakran tartalmaznak minimális, 1--2 banis (0.01--0.02 RON) órás különbségeket. Ez a finom gradiens a célfüggvény szempontjából elegendő ahhoz, hogy a solver rövid távon profitábilisnak ítélje az akkumulátor ismételt mozgatását, noha ez üzemi oldalról indokolatlan. Egy körülbelül 18 000 RON értékű, nagyjából 6000 ciklusra tervezett 10 kWh-s akkumulátor esetén az elhasználódás költsége nagyságrendileg 0.15 RON/kWh energiamozgatásra becsülhető. Vagyis egy látszólag nyereséges, de valójában túl gyakori töltési-kisütési művelet könnyen negatív nettó hasznot eredményezhet.

\textbf{A degradációs büntetőfüggvény bevezetése:}

A probléma kezelése érdekében a modell továbbfejlesztett változatában lineáris amortizációs költséget vezettünk be. Az új célfüggvény az adott nap teljes költségét az alábbi módon számítja:

\begin{equation}
C_{total} = \sum_{t=1}^{24} \left( P_{import}(t) \cdot p_{buy}(t) - P_{export}(t) \cdot p_{sell}(t) + \left| \Delta E_{batt}(t) \right| \cdot C_{deg} \right)
\end{equation}

Ahol:
\begin{itemize}
	\item $P_{import}(t)$ és $P_{export}(t)$ a hálózatból felvett és visszatáplált teljesítmény az adott $t$ órában,
	\item $p_{buy}(t)$ és $p_{sell}(t)$ az ENTSO-E órás árai,
	\item $\left| \Delta E_{batt}(t) \right|$ az akkumulátorba betöltött vagy onnan kivett energia abszolút értéke,
	\item $C_{deg}$ a konstans degradációs költség, amelyet a modellben $0.15$ RON/kWh értékkel közelítettünk.
\end{itemize}

\textbf{Az optimalizációs stratégia átalakulása:}

A büntetőparaméter bevezetése látványosan átrendezte az algoritmusok viselkedését. A mikrociklusok eltűntek, és az optimalizáló csak akkor használta az akkumulátort, amikor a nappali völgyidőszak és az esti csúcsidőszak közötti árkülönbség valóban indokolta az energiatárolás igénybevételét. Ha az arbitrázs kisebb volt, mint a degradációs küszöb, a solver inkább pihentette az akkumulátort, így a költségminimalizálás mellett a rendszer élettartamát is óvta.

Ez az apró módosítás mérnöki szempontból jelentős: a modell így már nem pusztán matematikailag optimális, hanem üzemeltetési oldalról is reálisabb válaszokat ad. A degradációs tag beépítése különösen fontos ott, ahol a döntéshozatal nem egyszeri optimumkeresés, hanem hosszú távú eszközhasználat és beruházás-védelem kérdése is.

\textbf{Algoritmusok közötti gyakorlati különbségek:}
...
\textbf{A jelenlegi modell gyakorlati értéke:}
